...
function StepContainer({ num, title, icon: Icon, isActive, isDone, onSelect, children }: {
  ...
  return (
    <div className={`border rounded-lg transition-all duration-300 ${isActive ? 'border-indigo-500/40 shadow-md shadow-indigo-500/10' : isDone ? 'border-emerald-500/30 bg-emerald-500/10' : 'border-white/10 bg-white/[0.02]'}`}>
      <div className="flex items-center gap-3 px-5 py-3.5 cursor-pointer hover:bg-black/[0.02] transition-colors" onClick={onSelect}>
        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${isDone ? 'bg-emerald-500 text-white' : isActive ? 'bg-indigo-600 text-white' : 'bg-white/5 text-slate-500'}`}>
          {isDone ? <Check className="w-4 h-4" /> : num}
        </div>
        <Icon className={`w-4 h-4 ${isActive ? 'text-indigo-300' : isDone ? 'text-emerald-300' : 'text-slate-500'}`} />
        <span className={`text-sm font-semibold ${isActive ? 'text-white' : isDone ? 'text-emerald-300' : 'text-slate-500'}`}>{title}</span>
        {isDone && <span className="text-[11px] text-emerald-300 font-medium ml-auto">완료</span>}
        {isActive && <span className="text-[11px] text-indigo-300 font-medium ml-auto">현재 단계</span>}
        {!isActive && !isDone && (
          <span className="text-[11px] text-slate-500 font-medium ml-auto flex items-center gap-0.5">
            <ChevronDown className="w-3 h-3" /> 펼치기
          </span>
        )}
      </div>
      {isActive && <div className="px-5 pb-5 pt-1 border-t border-white/10">{children}</div>}
    </div>
  )
}
  ...
  if (!editLoaded) {
    return (
      <div className="animate-fade-in py-24 text-center">
        <Loader2 className="w-8 h-8 text-indigo-300 animate-spin mx-auto mb-3" />
        <p className="text-sm text-slate-400">설교 데이터 불러오는 중...</p>
      </div>
    )
  }
  if (editError) {
    return (
      <div className="animate-fade-in py-24 text-center">
        ...
        <button onClick={() => router.push('/dashboard')} className="mt-4 text-sm text-indigo-300 underline">대시보드로 돌아가기</button>
      </div>
    )
  }
  ...
  return (
    <div className="animate-fade-in max-w-3xl mx-auto pb-16">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => router.push('/dashboard')} className="flex items-center gap-1.5 text-sm text-slate-400 hover:text-white transition-colors">
        ...
        <div className="flex items-center gap-1 ml-auto text-[11px] text-slate-400">
          {[1,2,3,4,5,6,7,8].map(st => (
            <div key={st} className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold cursor-pointer hover:opacity-80 transition-opacity ${stepDone(s, st) ? 'bg-emerald-500 text-white' : activeStep === st ? 'bg-indigo-600 text-white' : 'bg-white/5 text-slate-500'}`}
              onClick={() => goTo(st)}>
      ...
      {draftRestored && draftTimestamp && (
        <div className="flex items-center justify-between px-4 py-2.5 bg-amber-500/10 border border-amber-500/30 rounded-lg text-xs text-amber-300">
          ...
          <button type="button" onClick={() => { clearDraft(); setDraftRestored(false) }}
            className="underline hover:text-amber-200">닫기</button>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-4 gap-2">
            <div className="col-span-2">
              <label className="block text-xs font-medium text-slate-400 mb-1">성경 책</label>
              <input type="text" value={s.bibleBook} onChange={e => update({ bibleBook: e.target.value })}
                placeholder="예: 로마서" className="w-full px-3 py-2 text-sm border border-white/10 rounded-md bg-white/5 text-white focus:outline-none focus:ring-1 focus:ring-indigo-500/30" />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1">장</label>
              <input type="number" value={s.chapterStart} onChange={e => update({ chapterStart: e.target.value })}
                placeholder="8" className="w-full px-3 py-2 text-sm border border-white/10 rounded-md bg-white/5 text-white focus:outline-none focus:ring-1 focus:ring-indigo-500/30" />
            </div>
            <div className="grid grid-cols-2 gap-1">
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">시작 절</label>
                <input type="number" value={s.verseStart} onChange={e => update({ verseStart: e.target.value })}
                  placeholder="1" className="w-full px-3 py-2 text-sm border border-white/10 rounded-md bg-white/5 text-white focus:outline-none focus:ring-1 focus:ring-indigo-500/30" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">끝 절</label>
                <input type="number" value={s.verseEnd} onChange={e => update({ verseEnd: e.target.value })}
                  placeholder="11" className="w-full px-3 py-2 text-sm border border-white/10 rounded-md bg-white/5 text-white focus:outline-none focus:ring-1 focus:ring-indigo-500/30" />
              </div>
            </div>
          </div>

          <div className="flex items-center justify-between">
            <label className="block text-xs font-medium text-slate-400 mb-1.5">성경 본문</label>
            <button type="button" onClick={handleFetchBible} disabled={bibleLoading || !s.bibleBook || !s.chapterStart}
              className="text-xs font-medium text-indigo-300 hover:text-indigo-200 transition-colors flex items-center gap-1 disabled:opacity-50">
          ...
          <textarea value={s.bibleText} onChange={e => update({ bibleText: e.target.value })}
              ...
              className="w-full px-3 py-2 text-sm border border-white/10 rounded-md bg-white/5 text-white focus:outline-none focus:ring-1 focus:ring-indigo-500/30 resize-none font-mono leading-relaxed" />

          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1.5">📖 본문 관찰</label>
            <div className="bg-indigo-950/40 border border-indigo-500/20 rounded-md p-3 mb-2 space-y-1.5 text-[12px] text-indigo-300">
              <p className="font-medium text-indigo-300">본문을 천천히 읽으며 관찰해보세요:</p>
            ...
            <textarea value={s.observationNotes} onChange={e => update({ observationNotes: e.target.value })}
              ...
              className="w-full px-3 py-2 text-sm border border-white/10 rounded-md bg-white/5 text-white focus:outline-none focus:ring-1 focus:ring-indigo-500/30 resize-none" />
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-1">
              <label className="block text-xs font-medium text-slate-400">핵심 메시지</label>
            ...
            {suggestionsOpen === 'coreMessage' && suggestionsCache.coreMessage?.length > 0 && (
              <div className="absolute z-20 top-full mt-1 left-0 right-0 bg-[#0c1020] border border-white/10 rounded-lg shadow-lg p-2 space-y-1 max-h-56 overflow-y-auto">
                {suggestionsCache.coreMessage.map((item, i) => (
                  <button key={i} type="button" onClick={() => pickSuggestion('coreMessage', item.value)}
                    className="w-full text-left p-2 rounded-md hover:bg-indigo-500/15 transition-colors text-xs">
                    <span className="font-medium text-white">{item.value}</span>
                    {item.reason && <span className="block text-[11px] text-slate-400 mt-0.5">{item.reason}</span>}
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-1">
              <label className="block text-xs font-medium text-slate-400">설교 제목</label>
            ...
            <input type="text" value={s.title} onChange={e => update({ title: e.target.value })}
              ...
              className="w-full px-3 py-2 text-sm border border-white/10 rounded-md bg-white/5 text-white focus:outline-none focus:ring-1 focus:ring-indigo-500/30" />
          ...
          <div className="flex justify-end pt-1">
            <button type="button" onClick={() => goTo(3)}
              ...
              className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-md hover:bg-indigo-700 transition-colors disabled:opacity-50">
              다음 단계 <ChevronRight className="w-4 h-4" />
            </button>
          </div>
        </div>
      ...
      <StepContainer num={3} title="청중 분석 & 적용 방향" icon={Users}
        ...
        <div className="space-y-4">
          <div className="bg-emerald-950/40 border border-emerald-500/20 rounded-md p-3 space-y-1.5 text-[12px] text-emerald-300">
            <p className="font-medium text-emerald-300">🎯 청중을 고려할수록 설교가 살아납니다:</p>
            <ul className="list-disc pl-4 space-y-0.5">
      ...
      <StepContainer num={4} title="3대지 구조" icon={Layers}
        ...
        <div className="space-y-4">
          <p className="text-[12px] text-slate-400 leading-relaxed">본문의 흐름을 따라 3대지를 세웁니다. 각 대지는 문장형으로, 서로 중복되지 않고 논리적으로 연결되어야 합니다.</p>
          ...
          {s.outlinePoints.map((_, idx) => (
            <div key={idx} className="relative border border-white/10 rounded-md bg-white/5 text-white p-3 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold text-indigo-300 bg-indigo-600/10 px-2 py-0.5 rounded">대지 {idx + 1}</span>
                <div className="flex items-center gap-1">
                  {(['point', 'title', 'var'] as const).map(action => {
                    ...
                    return (
                      <button key={action} type="button" onClick={() => {
                        ...
                        className="text-[11px] text-indigo-300 hover:text-indigo-200 transition-colors disabled:opacity-30 flex items-center gap-0.5"
                        title={labels[action]}>
                  ...
                  {s.outlinePoints.length > 1 && (
                    <button type="button" onClick={() => removePoint(idx)}
                      className="text-[11px] text-rose-300 hover:text-rose-200 transition-colors ml-1">
              ...
              <input type="text" value={s.outlinePoints[idx] || ''} onChange={e => {
                ...
                className="w-full px-3 py-2 text-sm font-medium border border-white/10 rounded-md bg-white/5 text-white focus:outline-none focus:ring-1 focus:ring-indigo-500/30" />
              ...
              {(['point', 'title', 'var'] as const).map(action => {
                ...
                return (
                  <div key={key} className="absolute z-20 top-full mt-1 left-0 right-0 bg-[#0c1020] border border-white/10 rounded-lg shadow-lg p-2 space-y-1 max-h-56 overflow-y-auto">
          ...
          <button type="button" onClick={addPoint}
            className="w-full py-2.5 border-2 border-dashed border-white/10 rounded-md text-sm font-medium text-slate-400 hover:text-indigo-300 hover:border-indigo-500/40 transition-colors flex items-center justify-center gap-1.5">
            + 대지 추가
          </button>
          <div className="flex justify-end pt-1">
      ...
      <StepContainer num={5} title="대지별 전개 (본문해설 · 예화 · 적용)" icon={Pen}
        ...
        <div className="space-y-4">
          <p className="text-[12px] text-slate-400 leading-relaxed">각 대지를 본문해설, 예화, 적용으로 발전시킵니다. AI가 모든 대지를 한 번에 생성할 수 있습니다.</p>
          <div className="flex justify-end">
          ...
          {s.outlinePoints.map((pt, idx) => (
            <div key={idx} className="border border-white/10 rounded-md bg-white/5 text-white p-3 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold text-indigo-300 bg-indigo-600/10 px-2 py-0.5 rounded">
                  대지 {idx + 1}: {pt || '(제목 없음)'}
                </span>
                <button type="button" onClick={async () => {
                  ...
                  className="text-[11px] text-indigo-300 hover:text-indigo-200 transition-colors flex items-center gap-1 disabled:opacity-50">
                  {loading === 5 ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                  AI 생성
                </button>
              </div>
              <div>
                <label className="block text-[10px] font-medium text-slate-400 mb-0.5">본문해설</label>
              ...
              <div>
                <label className="block text-[10px] font-medium text-slate-400 mb-0.5">예화</label>
              ...
              <div>
                <label className="block text-[10px] font-medium text-slate-400 mb-0.5">적용</label>
      ...
      <StepContainer num={6} title="서론 & 결론" icon={FileText}
        ...
        <div className="space-y-4">
          <p className="text-[12px] text-slate-400 leading-relaxed">모든 내용을 바탕으로 서론과 결론을 작성합니다.</p>
          <div className="flex justify-end">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1">서론</label>
            <textarea value={s.introduction} onChange={e => update({ introduction: e.target.value })}
              rows={3} placeholder="회중을 본문으로 인도하는 서론 (3~5문장)"
              className="w-full px-3 py-2 text-sm border border-white/10 rounded-md bg-white/5 text-white focus:outline-none focus:ring-1 focus:ring-indigo-500/30 resize-none" />
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1">결론</label>
      ...
      <StepContainer num={7} title="완성 원고" icon={FileText}
        ...
        <div className="space-y-4">
          <p className="text-[12px] text-slate-400 leading-relaxed">지금까지 준비한 모든 내용을 하나의 완성된 설교 원고로 조립합니다. AI가 통합 원고를 생성하거나 직접 작성하세요.</p>

          {/* 설정 행: 스타일 · 분량 · 생성 버튼 */}
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-1.5">
              <span className="text-[11px] text-slate-400 font-medium">스타일</span>
              <select value={manuscriptStyle} onChange={e => setManuscriptStyle(e.target.value)}
                className="text-xs border border-white/10 rounded px-2 py-1 bg-white/5 text-slate-200 focus:outline-none focus:ring-1 focus:ring-indigo-500/30">
            ...
            <div className="flex items-center gap-1.5">
              <span className="text-[11px] text-slate-400 font-medium">분량</span>
          ...
          {s.manuscript && (
            <>
                ...
                <div className="flex items-center gap-1 ml-auto">
                  ...
                  {s.outlinePoints.filter(Boolean).map((pt, i) => (
                    <button key={i} type="button" onClick={() => handleRegenerateSection(pt)}
                      disabled={loading === 7}
                      className="text-xs px-1.5 py-0.5 rounded border border-white/10 hover:bg-white/5 disabled:opacity-50 transition-colors">
                      {i + 1}대지
                    </button>
                  ))}
                </div>
              ...
              <div className="flex items-center justify-between px-4 py-2.5 bg-white/5 border-b border-white/10">
                <span className="text-xs font-medium text-slate-300">📋 설교 원고 검증</span>
              ...
              {reviewResult && (
                <div className="p-4 space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      { key: 'gospel_centered', label: '복음 중심성', icon: '✝️', border: 'border-rose-500/20', bg: 'bg-rose-950/40' },
                      { key: 'biblical_faithfulness', label: '본문 충실성', icon: '📖', border: 'border-indigo-500/20', bg: 'bg-indigo-500/15' },
                      { key: 'application_specificity', label: '적용 구체성', icon: '🎯', border: 'border-emerald-500/20', bg: 'bg-emerald-950/40' },
                      { key: 'logical_flow', label: '흐름의 논리성', icon: '🔗', border: 'border-amber-500/20', bg: 'bg-amber-950/40' },
                      ...
                      return (
                        <div key={key} className={`p-3 rounded-lg border ${border} ${bg}`}>
                          ...
                          {item.feedback && <p className="text-[11px] text-slate-300 mb-1">{item.feedback}</p>}
                          {item.suggestion && (
                            <p className="text-[11px] text-slate-400 italic border-t border-white/10 pt-1 mt-1">
                              💡 {item.suggestion}
                            </p>
                          )}
                        </div>
                  ...
                  {reviewResult.overall && (
                    <div className="p-3 bg-white/5 rounded-lg">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs font-semibold text-slate-200">📊 종합 평가</span>
                        <span className="text-sm font-bold">{'★'.repeat(reviewResult.overall.score || 0)}{'☆'.repeat(5 - (reviewResult.overall.score || 0))}</span>
                      </div>
                      <p className="text-[11px] text-slate-300">{reviewResult.overall.summary}</p>
                    </div>
                  )}
                </div>
              )}
      ...
      <StepContainer num={8} title="태그 & 저장" icon={Tags}
        ...
        <div className="space-y-5">
          ...
          {/* AI 분석 버튼 */}
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-400">AI가 원고를 분석하여 태그 추천</span>
          ...
          {selectedTagIds.length > 0 && (
            <div className="flex flex-wrap gap-1.5 p-3 bg-indigo-950/40 border border-indigo-500/20 rounded-md">
              {selectedTagIds.map(id => {
                const t = ALL_THEMES.find(th => th.id === id)
                if (!t) return null
                return (
                  <span key={id} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-medium bg-indigo-500/15 border border-indigo-500/30 text-indigo-300">
                    {t.name}
                ...
                <button key={t.id} type="button" onClick={() => toggleTag(t.id)}
                  className={`px-2.5 py-1 rounded-full text-[11px] font-medium border transition-all ${
                    selectedTagIds.includes(t.id)
                      ...
                      : 'bg-white/5 border-white/10 text-slate-300 hover:border-orange-500/40 hover:text-orange-300'
                  }`}>
                  {t.name}
                </button>
              ))}
                ...
                <button key={t.id} type="button" onClick={() => toggleTag(t.id)}
                  className={`px-2.5 py-1 rounded-full text-[11px] font-medium border transition-all ${
                    selectedTagIds.includes(t.id)
                      ...
                      : 'bg-white/5 border-white/10 text-slate-300 hover:border-purple-500/40 hover:text-purple-300'
                  }`}>
                  {t.name}
                </button>
              ))}
          ...
          {/* 그래프 링크 */}
          <div className="flex items-center gap-2 p-3 bg-blue-950/40 border border-blue-500/20 rounded-md">
          ...
          <div className="flex justify-end pt-2 border-t border-white/10">
            <button type="button" onClick={handleSave} disabled={saving || !stepDone(s, 7)}
              className="flex items-center gap-1.5 px-6 py-2.5 bg-indigo-600 text-white text-sm font-medium rounded-md hover:bg-indigo-700 transition-colors disabled:opacity-50">
