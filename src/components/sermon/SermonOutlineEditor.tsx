...
export default function SermonOutlineEditor({ outline, onChange }: SermonOutlineEditorProps) {
  ...
  return (
    <div className="space-y-5">
      <h3 className="text-[15px] font-extrabold text-slate-800">설교 개요</h3>
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[13px] font-bold text-slate-600">서론</span>
        </div>
        <textarea
          value={introduction}
          onChange={e => updateIntroduction(e.target.value)}
          placeholder="서론을 입력하세요"
          className="w-full min-h-[80px] px-3.5 py-2.5 rounded-xl bg-white border border-slate-200 text-[14px] text-slate-700 placeholder-slate-300 resize-y focus:outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 transition-all"
        />
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[13px] font-bold text-slate-600">본론</span>
          </div>
          <button
            onClick={addPoint}
            className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-indigo-50 text-indigo-600 text-[12px] font-bold hover:bg-indigo-100 transition-all active:scale-95"
          >
            ...
            <div key={i} className="p-4 rounded-xl bg-white border border-slate-200 hover:border-indigo-200 transition-all">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-bold text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded-lg">포인트 {i + 1}</span>
                </div>
                <button
                  onClick={() => removePoint(i)}
                  className="w-7 h-7 rounded-lg hover:bg-rose-50 flex items-center justify-center transition-all text-slate-300 hover:text-rose-500"
                >
              ...
              <input
                ...
                placeholder="소제목"
                className="w-full mb-2 px-3 py-2 rounded-lg bg-slate-50 border border-slate-200 text-[14px] font-bold text-slate-700 placeholder-slate-300 focus:outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 transition-all"
              />
              <textarea
                value={point.content}
                onChange={e => updatePoint(i, 'content', e.target.value)}
                placeholder="내용을 입력하세요"
                className="w-full min-h-[80px] px-3 py-2 rounded-lg bg-slate-50 border border-slate-200 text-[14px] text-slate-700 placeholder-slate-300 resize-y focus:outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 transition-all"
              />
              <div className="grid grid-cols-2 gap-2 mt-2">
                <input
                  ...
                  className="px-2.5 py-2 rounded-lg bg-slate-50 border border-slate-200 text-[12px] text-slate-600 placeholder-slate-300 focus:outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 transition-all"
                />
                <input
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-full bg-gradient-to-br from-emerald-500 to-green-600 flex items-center justify-center shrink-0">
            <span className="text-[11px] font-bold text-white">결</span>
          </div>
          <span className="text-[13px] font-bold text-slate-600">결론</span>
        </div>
        <textarea
          value={conclusion}
