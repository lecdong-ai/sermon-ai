...
import { Upload, FileText, AlertCircle, CheckCircle, X, Loader2, Sparkles, ArrowRight, BookOpen, Users, Film, Presentation } from 'lucide-react'
...
const VALUE_PROPS = [
  ...
  { icon: Users, label: '소그룹 나눔' },
  { icon: Film, label: '설교 대본' },
  { icon: Presentation, label: '쇼츠 대본' },
]
...
export default memo(function FileUpload({ onSuccess, dark }: Props) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      const res = await fetch('/school/api/upload', { method: 'POST', body: formData })
        ...
        <div>
          ...
          {theme === 'dark' && phase === 'idle' && (
            <div className="mt-6 text-center">
              <p className="text-[13px] text-white/30 mb-3">업로드하면 AI가 4종 콘텐츠를 자동 생성합니다</p>
