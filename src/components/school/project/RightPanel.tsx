...
import { ProjectDetail, PROJECT_STATUS_LABELS, PROJECT_STATUS_ORDER, type ProjectStatus } from '@/lib/school/project/types'
import SaveStatusIndicator from '@/components/school/project/shared/SaveStatusIndicator'
import VersionHistoryDrawer from '@/components/school/project/shared/VersionHistoryDrawer'
import RecentChangesPanel from '@/components/school/project/shared/RecentChangesPanel'
import StatusTimeline from '@/components/school/project/shared/StatusTimeline'
import { MOCK_SAVE_STATE, MOCK_VERSIONS, MOCK_RECENT_CHANGES } from '@/lib/school/project/statusData'
import { NOTE_TYPE_LABELS, NOTE_TYPE_DOTS, type NoteEntry } from '@/lib/school/project/notesData'
...
export default function RightPanel({ project, activeTab, onProjectUpdated, updateStatus }: Props) {
  ...
  useEffect(() => {
    ...
    fetch('/school/api/insights', { signal: ac.signal })
      .then((r) => r.json())
      .then((json) => {
        if (json.success) {
  ...
  const handleUnlink = async (insightId: string) => {
    ...
    try {
      const res = await fetch(`/school/api/insights/${insightId}`, {
        method: 'PATCH',
      ...
      <aside className="w-72 border-l border-white/5 bg-[#04060f]/60 flex flex-col shrink-0 overflow-y-auto scrollbar-thin">
        ...
        <div className="p-4 border-b border-white/5">
          ...
          <div className="space-y-1">
            <QuickAction label="연결된 노트 보기" onClick={() => router.push('/school/advanced/notes')} />
            <QuickAction label="유사 설교 검색" onClick={() => router.push('/school/advanced/archive')} />
            {project.seriesName && (
        ...
        <div className="p-4">
          ...
          <div className="space-y-1">
            <QuickAction label="그래프로 보기" onClick={() => router.push(`/projects/${project.id}?tab=connections`)} />
            <QuickAction label="아카이브 검색" onClick={() => router.push('/school/advanced/archive')} />
          </div>
        </div>
      </aside>
