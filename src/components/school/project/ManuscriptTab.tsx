...
import { EMPTY_MANUSCRIPT } from '@/lib/school/project/johnManuscriptData'
import type { SermonSection, IllustrationNote, ReferenceNote, JohnManuscriptData } from '@/lib/school/project/johnManuscriptData'
import { AppSectionHeader } from '@/components/school/project/shared'
...
import { detectCurrentStage } from '@/lib/school/project/stageChecker'
import { PROJECT_STATUS_ORDER, type ProjectStatus } from '@/lib/school/project/types'
import {
  MANUSCRIPT_VERSIONS as JOHN_MANUSCRIPT_VERSIONS,
  RECENT_ACTIVITY as JOHN_RECENT_ACTIVITY,
} from '@/lib/school/project/johnVersionData'
import type { ManuscriptVersion } from '@/lib/school/project/johnVersionData'
import { getStorageItem, setStorageItem } from '@/lib/school/storage'
import { readProjectCore } from '@/lib/school/project/projectStorage'
...
export default function ManuscriptTab({ project, onStatusUpdate }: Props) {
  ...
  const doSave = useCallback(async (manuscriptToSave: JohnManuscriptData) => {
    ...
    try {
      ...
      const res = await fetch('/school/api/sermons/' + project.id, {
        method: 'PUT',
  ...
  const handleSendToWorkspace = useCallback(async () => {
    ...
    try {
      ...
      const passageText = project.passages && project.passages.length > 0
        ...
        : project.passage
      const res = await fetch('/school/api/sermons', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    />
      ...
      <div className="flex flex-1 min-h-0">
        ...
        {showPrepPanel && (
          <PrepSummaryPanel
            manuscript={manuscript}
            onGoToPrep={() => router.push(`/projects/${project.id}?tab=prep`)}
      ...
      <WritingStatusBar
        ...
        onGoToVersions={() => router.push(`/projects/${project.id}?tab=versions`)}
        onGoToPrep={() => router.push(`/projects/${project.id}?tab=prep`)}
      />
