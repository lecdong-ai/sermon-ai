...
export default function QtQuickIndexNavPortrait({
  ...
  return (
    ...
    >
      {/* Upper Ribbon: Active tab is prominent & bold, inactive tabs are muted */}
      <div className="flex items-center space-x-1.5 bg-white/95 px-2 py-1 rounded-xl border border-slate-200/90 shadow-md backdrop-blur-xs">
          ...
          <button
            ...
            className={`px-2.5 py-0.5 rounded text-[9px] font-black transition-all cursor-pointer ${
              activeTab === 'yearlygrid'
                ? 'text-white shadow-md border-b-2 border-amber-300 scale-110 font-black z-10'
                : 'text-slate-400 opacity-70 hover:opacity-100 hover:bg-slate-100 hover:text-slate-800'
            }`}
        ...
        <div className="flex items-center space-x-1 px-0.5">
          <button type="button" data-nav-target="habit" className="text-[10px] hover:scale-130 transition-transform cursor-pointer" title="습관">🌱</button>
          <button type="button" data-nav-target="gratitude" className="text-[10px] hover:scale-130 transition-transform cursor-pointer" title="감사">☀️</button>
          <button type="button" data-nav-target="budget" className="text-[10px] hover:scale-130 transition-transform cursor-pointer" title="가계부">💰</button>
          <button type="button" data-nav-target="kpt" className="text-[10px] hover:scale-130 transition-transform cursor-pointer" title="KPT">🔄</button>
        </div>
            ...
            return (
              <button
                ...
                className={`px-1.5 py-0.5 rounded text-[8px] font-extrabold transition-all cursor-pointer ${
                  isCurrentW
                    ? 'bg-slate-900 text-white font-black shadow-md border-b-2 border-amber-400 scale-110 z-10'
                    : 'text-slate-400 opacity-70 hover:opacity-100 hover:bg-slate-100 hover:text-slate-800'
                }`}
              >
                W{wNo}
      ...
      {/* Lower Ribbon: Active Month is prominent, inactive months are small & muted */}
          ...
          return (
            <button
              ...
              className={`py-0.5 text-center transition-all relative cursor-pointer ${
                isSelectedM
                  ? 'w-5.5 text-[8.5px] text-white font-black shadow-md scale-115 border-b-2 border-amber-300 z-10 rounded'
                  : 'w-4 text-[7px] text-slate-500 opacity-60 hover:opacity-100 hover:w-5'
              }`}
              style={{
