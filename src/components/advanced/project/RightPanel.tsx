...
export default function RightPanel({ project, activeTab }: Props) {
  ...
  return (
    <>
      ...
      <aside className="w-72 border-l border-paper-200 bg-paper-50 flex flex-col shrink-0 overflow-y-auto scrollbar-thin">

        {/* 프로젝트 정보 */}
        <div className="p-4 border-b border-paper-200">
          <h3 className="text-[10px] font-semibold text-paper-400 uppercase tracking-widest mb-3">프로젝트 정보</h3>
          <div className="space-y-2">
            ...
            {project.seriesName && (
              <div className="flex items-center gap-2">
                <span className="text-xs text-paper-400 w-14 shrink-0">시리즈</span>
                <span
                  className="text-xs text-paper-700 cursor-pointer hover:text-green-600 transition-colors"
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-paper-400 w-14 shrink-0">저장</span>
        ...
        {/* 상태 타임라인 */}
        <div className="p-4 border-b border-paper-200">
          <h3 className="text-[10px] font-semibold text-paper-400 uppercase tracking-widest mb-3">진행 단계</h3>
          <StatusTimeline currentStatus={project.status} />
          <div className="mt-2">
            <div className="adv-progress-bar h-1.5">
              <div className="adv-progress-fill bg-green-500 h-full rounded-full" style={{ width: `${progressPercent}%` }} />
            </div>
          </div>
        </div>
        ...
        {/* 통계 */}
        <div className="p-4 border-b border-paper-200">
          <h3 className="text-[10px] font-semibold text-paper-400 uppercase tracking-widest mb-3">통계</h3>
        ...
        <div className="p-4 border-b border-paper-200">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-[10px] font-semibold text-paper-400 uppercase tracking-widest">버전 기록</h3>
            <button
        ...
        {project.themeNames.length > 0 && (
          <div className="p-4 border-b border-paper-200">
            <h3 className="text-[10px] font-semibold text-paper-400 uppercase tracking-widest mb-3">주제</h3>
            <div className="flex flex-wrap gap-1.5">
              {project.themeNames.map(t => (
                <span key={t} className="text-[11px] px-2 py-0.5 rounded-full bg-green-100 text-green-700">
                  {t}
                </span>
              ))}
            </div>
        ...
        {project.tagNames.length > 0 && (
          <div className="p-4 border-b border-paper-200">
            <h3 className="text-[10px] font-semibold text-paper-400 uppercase tracking-widest mb-3">태그</h3>
            <div className="flex flex-wrap gap-1.5">
              {project.tagNames.map(t => (
                <span key={t} className="text-[11px] px-2 py-0.5 rounded-md bg-paper-150 text-paper-600">
                  #{t}
                </span>
              ))}
            </div>
        ...
        {/* 관련 자료 연결 */}
        <div className="p-4 border-b border-paper-200">
          <h3 className="text-[10px] font-semibold text-paper-400 uppercase tracking-widest mb-3">관련 자료</h3>
        ...
        {/* 빠른 작업 */}
        <div className="p-4">
          <h3 className="text-[10px] font-semibold text-paper-400 uppercase tracking-widest mb-3">빠른 작업</h3>
          <div className="space-y-1">
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs text-paper-400 w-14 shrink-0">{label}</span>
      <span className="text-xs text-paper-700 truncate">{value}</span>
    </div>
  )
}
...
function StatItem({ value, label }: { value: string; label: string }) {
  return (
    <div className="bg-white rounded-lg border border-paper-200 p-2.5 text-center">
      <div className="text-sm font-bold text-paper-800">{value}</div>
...
function QuickAction({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button onClick={onClick}
      className="w-full text-left text-xs text-paper-600 hover:text-green-600 hover:bg-green-50/50 rounded-md px-3 py-2 transition-colors">
      {label}
    </button>
  )
}
