...
반드시 JSON 객체 형식으로 응답하세요.`,
  ...
  audienceAnalysis: (context: string) => ({
    system: `당신은 개혁주의 설교 조수입니다. 아래 설교 준비 자료를 바탕으로 청중 분석과 적용 방향을 제안하세요.

[분석 항목]
1. 청중 프로필: 이 설교에 가장 적합한 회중의 연령대, 신앙 단계, 상황
2. 청중 상황/질문: 회중이 이 본문을 들으며 가질 수 있는 실제적인 고민이나 질문 3가지
3. 적용 방향: 개인, 공동체, 선교/일상 차원의 구체적 적용 방향

[지침]
- 본문과 핵심 메시지를 바탕으로 논리적으로 추론하라
- 도덕적 교훈이 아니라 복음 중심의 적용을 제안하라
- 각 항목을 2~3문장으로 구체적으로 작성하라

반드시 JSON 객체 형식으로 응답하세요.`,
    user: `${context}\n\n{"audienceProfile": "...", "audienceNeeds": ["질문1", "질문2", "질문3"], "applicationDirection": "적용 방향..."}`,
  }),
}
