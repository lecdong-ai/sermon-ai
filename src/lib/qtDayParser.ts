...
const SECTION_HEADERS: [RegExp, keyof ParsedDay][] = [
  ...
  [/(?:#+|\*{1,2})\s*(?:\d+[\.\)]\s*)?본문\s*관찰\s*[:*]?/i, 'observation'],
  [/(?:#+|\*{1,2})\s*(?:\d+[\.\)]\s*)?원어\s*핵심\s*[:*]?/i, 'originalWords'],
  [/(?:#+|\*{1,2})\s*(?:\d+[\.\)]\s*)?영어\s*핵심\s*[:*]?/i, 'englishWords'],
  [/(?:#+|\*{1,2})\s*(?:\d+[\.\)]\s*)?말씀\s*이해\s*[:*]?/i, 'understanding'],
  [/(?:#+|\*{1,2})\s*(?:\d+[\.\)]\s*)?복음으로\s*보기\s*[:*]?/i, 'gospel'],
  [/(?:#+|\*{1,2})\s*(?:\d+[\.\)]\s*)?나를\s*비추\s*[:*]?/i, 'reflection'],
  [/(?:#+|\*{1,2})\s*(?:\d+[\.\)]\s*)?오늘의\s*적용\s*[:*]?/i, 'application'],
  [/(?:#+|\*{1,2})\s*(?:\d+[\.\)]\s*)?영어로\s*붙드는\s*말씀\s*[:*]?/i, 'englishVerse'],
  [/(?:#+|\*{1,2})\s*(?:\d+[\.\)]\s*)?공동체\s*연결\s*[:*]?/i, 'community'],
  [/(?:#+|\*{1,2})\s*(?:\d+[\.\)]\s*)?오늘의\s*기도\s*[:*]?/i, 'prayer'],
  [/(?:#+|\*{1,2})\s*(?:\d+[\.\)]\s*)?한\s*줄\s*[:*]?/i, 'oneLine'],
  [/(?:#+|\*{1,2})\s*(?:\d+[\.\)]\s*)?인도자\s*[:*]?/i, 'leaderGuide'],
]
