...
export async function GET(request: NextRequest) {
  try {
    ...
    const maxVerses = data
      .filter(v => v.book === shortName && v.chapter === chapter)
      .reduce((max, v) => Math.max(max, v.verse), 0)

    if (maxVerses === 0) {
      return NextResponse.json({ success: false, error: '해당 장을 찾을 수 없습니다' }, { status: 404 })
    }

    return NextResponse.json({ success: true, maxVerses })
  } catch (e: any) {
    console.error('[bible/max-verses] Error:', e)
