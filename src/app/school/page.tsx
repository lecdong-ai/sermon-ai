...
import {
  ...
  ChevronDown, UserCheck, GraduationCap, Handshake, Wand2, Bell,
  QrCode, Layers, PenLine, Zap,
} from 'lucide-react';
...
function Reveal({ children, className = '', delay = 0 }: { children: ReactNode; className?: string; delay?: number }) {
  ...
  return (
    <div ref={ref} className={`school-reveal ${vis ? 'revealed' : ''} ${className}`} style={delay ? { transitionDelay: `${delay}ms` } : undefined}>
      {children}
    </div>
  );
}
...
const FEATURES = [
  {
    ...
    icon: Wand2, color: 'mint', bg: 'bg-mint-50', href: '/school/notice-writer', tag: '소통',
    large: true,
  },
  {
    title: '설교 워크스페이스',
    desc: '설교 업로드 하나로 요약·토론질문·원고·숏폼·카드뉴스까지.',
    icon: Layers, color: 'orange', bg: 'bg-orange-50', href: '/school/workspace', tag: '성장',
  },
  {
    title: 'PPT 스튜디오',
    desc: 'AI가 10가지 레이아웃으로 슬라이드 배치. 템플릿 색상·폰트 자동 적용.',
    icon: Presentation, color: 'navy', bg: 'bg-navy-50', href: '/school/ppt-studio', tag: '준비',
  },
  {
    title: '행사 신청 시스템',
    desc: '맞춤 필드·QR 체크인·형제자매 자동완성·CSV 내보내기.',
    icon: QrCode, color: 'rose', bg: 'bg-rose-50', href: '/school/events/manage', tag: '운영',
  },
  {
    title: '설교 프로젝트',
    desc: '성경연구→준비→작성→연결보기. 6단계 스튜디오 + AI 초안.',
    icon: PenLine, color: 'indigo', bg: 'bg-indigo-50', href: '/school/projects', tag: '깊이',
  },
];
...
const FREE_FEATURES = [
  ...
  'AI 공지문 작성기 무제한 이용',
  '설교 워크스페이스 — 요약·토론·원고·카드뉴스',
  'PPT 스튜디오 — 10개 레이아웃, 템플릿 자동 적용',
  '행사 신청 시스템 — QR 체크인, CSV 내보내기',
  '보관함 영구 저장 및 즐겨찾기 지원',
            ...
            <Reveal delay={300}>
              <div className="h-full p-7 rounded-2xl bg-gradient-to-br from-navy-900 to-navy-800 text-white flex flex-col justify-center items-center text-center relative overflow-hidden shine-hover">
                ...
                <div className="relative z-10">
                  ...
                  <Link href="/school/notice-writer" className="inline-flex items-center gap-1.5 text-xs font-bold text-mint-300 hover:text-mint-200 group">
                    시작하기 <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" />
                  </Link>
                </div>
              </div>
            </Reveal>
        ...
        <div className="container-custom max-w-5xl">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
            ...
            <Reveal className="lg:col-span-5 space-y-5">
              ...
              <div className="pt-3">
                <Link href="/school/notice-writer" className="group btn-secondary btn-sm">
                  작성기 사용해보기
            ...
            {FREE_ITEMS.map((item, i) => (
              ...
              >
                ...
                <div className="flex items-center justify-between mt-5 pt-4 border-t border-warm-200">
                  ...
                  <Link href="/school/notice-writer" className="text-xs font-bold text-mint-600 hover:text-mint-700 flex items-center gap-1 group-hover:gap-1.5 transition-all">
      ...
      <section className="section bg-warm-50">
        <div className="container-custom">
          <Reveal className="text-center mb-14">
            ...
            <h2 className="section-title">모든 기능을 무료로 사용하세요</h2>
            <p className="section-subtitle mx-auto">별도의 결제나 구독 없이 모든 서비스를 제한 없이 이용하실 수 있습니다.</p>
          </Reveal>

          <Reveal className="max-w-2xl mx-auto">
            <div className="bg-white rounded-3xl p-8 md:p-10 border-2 border-mint-300 shadow-lg ring-4 ring-mint-400/10 text-center space-y-6">
              ...
              <div>
                <h3 className="text-2xl font-black text-navy-950">무료 플랜</h3>
                <div className="mt-2">
              ...
              <Link
                href="/school/notice-writer"
                className="inline-block w-full max-w-xs mx-auto text-center py-4 rounded-2xl text-sm font-bold bg-mint-500 text-white hover:bg-mint-600 shadow-md shadow-mint-500/10 transition-all"
              >
                무료로 시작하기
              </Link>
            </div>
