...
export default function ManageEventsPage() {
  ...
  const fetchEvents = async () => {
    setLoading(true)
    const res = await fetch(`/school/api/manage/events?templates=${showTemplates}`)
  ...
  const handleClone = async (id: string) => {
    ...
    const res = await fetch(`/school/api/manage/events/${id}/clone`, { method: 'POST' })
    const data = await res.json()
  ...
  const handleDelete = async (id: string) => {
    ...
    const res = await fetch(`/school/api/manage/events/${id}`, { method: 'DELETE' })
  ...
  return (
    <div className="container-custom py-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <Link href="/school/events/manage/new" className="btn-primary btn-sm">
