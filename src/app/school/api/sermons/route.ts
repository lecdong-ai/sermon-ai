...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = projectSupabaseAdmin
      .from('sermons')
    ...
    const msg = String(err?.message || err || '조회 실패')
    const isConnectionError = /fetch failed|ENOTFOUND|ECONNREFUSED|ETIMEDOUT|getaddrinfo/i.test(msg)
    const userMessage = isConnectionError
      ? '설교 프로젝트 데이터베이스에 연결할 수 없습니다. 잠시 후 다시 시도하거나, 문제가 지속되면 support@churchschool.kr로 문의해주세요.'
      : (msg || '조회 실패')
