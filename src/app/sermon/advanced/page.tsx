...
import { ArrowLeft, BookOpen, Sparkles, Loader2, Check, Copy, RefreshCw, ChevronLeft, ChevronRight, AlertCircle, Target, Eye, PartyPopper, Trophy, Lightbulb, ListOrdered, MessageSquare } from 'lucide-react'
...
export default function AdvancedSermonPage() {
  ...
  const handleStep4 = async () => {
    ...
    const data = await callStep(4); if (data) setStep4Result(data)
  }
  const handleStep5 = async () => {
    const data = await callStep(5); if (data) setStep5Result(data)
  }
  const handleStep6 = async () => {
    const data = await callStep(6); if (data) setStep6Result(data)
  }
    ...
    try {
      const res = await fetch('/api/sermons/advanced', {
        ...
        body: JSON.stringify({
          ...
          core_message: coreMessage, main_points: step3Result?.main_points || [],
          expanded_points: step4Result?.expanded_points || [],
          applications: step5Result?.applications || [],
          conclusion_data: step6Result ? JSON.stringify(step6Result) : '',
        }),
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-[18px] font-extrabold text-slate-800">실전형 설교 준비</span>
          ...
          <div className="flex items-center gap-2 shrink-0">
            {finalText && (
              <button onClick={async () => { await navigator.clipboard.writeText(finalText); setCopied(true); setTimeout(() => setCopied(false), 2000) }}
                className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-[13px] font-bold transition-all hover:scale-105 active:scale-95"
              >{copied ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}{copied ? '복사됨' : '복사'}</button>
            )}
            <button onClick={() => router.push('/sermon')} className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-indigo-600 to-blue-600 text-white text-[13px] font-bold hover:shadow-lg hover:shadow-indigo-500/20 transition-all hover:scale-105 active:scale-95">
              <Eye className="w-4 h-4" />목록으로
            </button>
          </div>
        </div>
                  ...
                  <div key={s.id} className="relative">
                    ...
                    <div className="relative flex items-center gap-2 px-2 py-2 rounded-xl transition-all duration-200">
                      <div className={`w-[28px] h-[28px] rounded-xl flex items-center justify-center text-[11px] font-bold shrink-0 transition-all duration-300 ${
                        ...
                        step === s.id ? 'bg-gradient-to-br from-amber-500 to-orange-500 text-white shadow-sm' : 'bg-slate-100 text-slate-400'
                      }`}>
        ...
        <main className="flex-1 min-w-0 p-4">
          ...
          {(step === 2 || step === 3 || step === 4 || step === 5 || step === 6) && (
            <div className="glass-panel rounded-2xl p-6 border border-white/60">
                ...
                <>
                  ...
                  {!step2Result ? (
                    <button onClick={handleStep2} disabled={loading}
                      className="w-full py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-blue-600 text-white text-[14px] font-bold hover:shadow-lg hover:shadow-indigo-500/20 transition-all hover:scale-[1.01] active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2"
                  ...
                  {!step3Result ? (
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="p-4 rounded-xl bg-slate-50/50 border border-slate-200">
                        ...
                        <textarea value={step3Result.direction || ''} onChange={e => setStep3Result({...step3Result, direction: e.target.value})} className="w-full text-[14px] text-slate-700 bg-transparent border-none resize-none focus:outline-none" rows={2} />
                      </div>
                ...
                <>
                  ...
                  {!step4Result ? (
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="space-y-3">
                          <div className="p-4 rounded-xl bg-indigo-50/50 border border-indigo-100">
                            ...
                            <textarea value={step4Result.expanded_points[activePoint].exposition || ''}
                              ...
                              className="w-full text-[14px] text-slate-700 bg-transparent border-none resize-y focus:outline-none min-h-[80px]" />
                          </div>
                          <div className="p-4 rounded-xl bg-slate-50/50 border border-slate-200">
                      ...
                      <button onClick={handleStep4} disabled={loading} className="flex items-center gap-1.5 text-[13px] text-indigo-600 font-bold">{<RefreshCw className="w-3.5 h-3.5" />}재생성</button>
                    </div>
                  )}
                </>
              )}
              {step === 5 && (
                <>
                  <h3 className="text-[16px] font-extrabold text-slate-800 mb-1 flex items-center gap-2"><Target className="w-4 h-4 text-indigo-500" />적용 / 예화</h3>
                    ...
                    <div className="space-y-3">
                      ...
                      {step5Result.applications?.[activePoint] && (
                        <div className="space-y-3">
                          ...
                          <div className="p-4 rounded-xl bg-amber-50/50 border border-amber-100">
                            <p className="text-[11px] font-bold text-amber-500 mb-1">예화/사례</p>
                      ...
                      <button onClick={handleStep5} disabled={loading} className="flex items-center gap-1.5 text-[13px] text-indigo-600 font-bold">{<RefreshCw className="w-3.5 h-3.5" />}재생성</button>
                    </div>
                  )}
                </>
              )}
              {step === 6 && (
                <>
                  <h3 className="text-[16px] font-extrabold text-slate-800 mb-1 flex items-center gap-2"><Target className="w-4 h-4 text-indigo-500" />결론</h3>
                  ...
                  {!step6Result ? (
                    ...
                    <div className="space-y-3">
                      ...
                      <button onClick={handleStep6} disabled={loading} className="flex items-center gap-1.5 text-[13px] text-indigo-600 font-bold">{<RefreshCw className="w-3.5 h-3.5" />}재생성</button>
                    </div>
                  )}
                </>
              )}
          ...
          {step === 7 && (
            <div className="glass-panel rounded-2xl p-6 border border-white/60">
              <h3 className="text-[16px] font-extrabold text-slate-800 mb-1 flex items-center gap-2"><Eye className="w-4 h-4 text-indigo-500" />최종 설교원고</h3>
              ...
              {!finalText ? (
                <button onClick={handleFinal} disabled={loading}
                  className="w-full py-4 rounded-2xl bg-gradient-to-r from-indigo-600 to-blue-600 text-white text-[16px] font-bold hover:shadow-lg hover:shadow-indigo-500/20 transition-all hover:scale-[1.01] active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2"
                ...
                <div className="space-y-3">
                  ...
                  <button onClick={handleFinal} disabled={loading} className="flex items-center gap-1.5 text-[13px] text-indigo-600 font-bold">{<RefreshCw className="w-3.5 h-3.5" />}다시 생성</button>
                </div>
              )}
            </div>
          )}
            ...
            {step < 7 ? (
              step === 1 ? (
                <button onClick={handleStep1Next} disabled={!passage.trim() || loading}
                  className="flex items-center gap-1.5 px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-blue-600 text-white text-[13px] font-bold hover:shadow-lg hover:shadow-indigo-500/20 transition-all hover:scale-[1.01] active:scale-[0.98] disabled:opacity-50"
