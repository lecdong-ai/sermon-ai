  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-[22px] font-extrabold text-slate-800 mb-1">구독 관리</h1>
          <p className="text-[14px] text-slate-500">전체 {subs.length}개의 구독</p>
        </div>
        <button
          onClick={fetchSubs}
          className="flex items-center gap-2 px-4 py-2 rounded-xl border border-slate-200 bg-white text-[13px] font-semibold text-slate-600 hover:bg-slate-50 transition-all"
        >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-[13px]">
            ...
            <tbody>
              {subs.map((s, i) => (
                <tr key={s.id} className={`border-b border-slate-100 hover:bg-slate-50/50 transition-colors ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                  <td className="px-5 py-3">
                    <span className="font-mono text-[12px] text-slate-500">{s.user_id.slice(0, 8)}...</span>
                  </td>
                  <td className="px-5 py-3">
                    <span className="font-bold text-slate-700 capitalize">{s.plan}</span>
                  </td>
                  <td className="px-5 py-3">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-[12px] font-bold ${statusStyles[s.status] || 'bg-slate-100 text-slate-500'}`}>
                      {s.status}
                    </span>
                  </td>
                  <td className="px-5 py-3">
