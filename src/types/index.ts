  ...
  json_schema: {
    ...
    schema: {
      ...
      properties: {
        ...
        intro: { type: 'string', description: '서론 — 본문 배경과 인간 문제, 복음의 대답을 포함하여 설교 도입부 작성. 600~900자' },
        body: { type: 'string', description: '본론 — 3대지 설교형. 각 대지: 제목(짧고 명확) + 설명(2~4문장) + ▶ 핵심 의미: .... 가독성 좋게 줄바꿈 충분히. 1000~1500자' },
        conclusion: { type: 'string', description: '결론 — 설교의 중심 진술이 한눈에 보이게 종합. 300~500자' },
        application: { type: 'string', description: '적용 — 개인 적용과 공동체 적용을 구분하여 구체적 실천 방법 제시. 400~600자' },
