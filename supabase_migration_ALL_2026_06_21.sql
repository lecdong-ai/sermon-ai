...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'sermon_derivatives_type_check') THEN
    ALTER TABLE sermon_derivatives ADD CONSTRAINT sermon_derivatives_type_check
      CHECK (type IN ('summary', 'questions', 'cardnews', 'shorts', 'ppt', 'guide'));
  END IF;
END $$;
...
CREATE POLICY "sermon_derivatives_select" ON sermon_derivatives FOR SELECT USING (user_id = auth.uid());
DROP POLICY IF EXISTS "sermon_derivatives_insert" ON sermon_derivatives;
CREATE POLICY "sermon_derivatives_insert" ON sermon_derivatives FOR INSERT WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS "sermon_derivatives_update" ON sermon_derivatives;
CREATE POLICY "sermon_derivatives_update" ON sermon_derivatives FOR UPDATE USING (user_id = auth.uid());
DROP POLICY IF EXISTS "sermon_derivatives_delete" ON sermon_derivatives;
CREATE POLICY "sermon_derivatives_delete" ON sermon_derivatives FOR DELETE USING (user_id = auth.uid());

-- =============================================
-- 완료!
-- =============================================
-- 결과: "Success. No rows returned" 가 보여야 정상
-- 4개 테이블 모두 생성 + RLS 설정 완료
