...
import type { PptSlide, PptTextStyle } from '@/types/workspace'
import { getTemplates, applyTemplate, type TemplateRecord } from '@/lib/templateRegistry'
import { recommendTemplate } from '@/lib/workspace/openai'
    ...
    <div className="flex h-[calc(100vh-4rem)] bg-[#F0F4FA]">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
          ...
          <aside className="w-80 shrink-0 border-l border-[#D0D8E5] bg-white overflow-y-auto">
            <div className="p-4 space-y-5">
              <div>
                <h3 className="text-[13px] font-bold text-[#1A2A3A] mb-3 flex items-center gap-1.5">
                  <Sparkles className="w-4 h-4 text-[#1B3A5C]" />
                  AI 생성
                </h3>
                <button
                  onClick={handleGenerate}
                  disabled={generating || !sermon?.raw_text}
                  className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-[14px] font-bold bg-gradient-to-r from-[#1B3A5C] to-[#0F2B4F] text-white hover:from-[#0F2B4F] hover:to-[#0A1F3A] disabled:opacity-40 transition-all duration-200"
                >
                  {generating ? (
              ...
              {slides.length > 0 && (
                <div>
                  <h3 className="text-[13px] font-bold text-[#1A2A3A] mb-3 flex items-center gap-1.5">
                    <Settings className="w-4 h-4 text-[#1B3A5C]" />
                    템플릿
                  </h3>
                  <div className="grid grid-cols-2 gap-2">
                    {(templates.length > 0 ? templates : [{ id: 'modern', name: '모던', primary_color: '1B3A5C', accent_color: '4A90D9', background_color: 'FFFFFF', text_color: '1A1A2E', font_title: 'Malgun Gothic', font_body: 'Malgun Gothic', gradient: 'from-[#1B3A5C] to-[#4A90D9]', ai_guide: null, file_url: null, is_active: true, category: 'general' }]).map((t) => (
                      <button
                        ...
                        className={`px-3 py-2.5 rounded-xl text-[12px] font-medium transition-all border ${selectedTemplateId === t.id ? 'border-[#1B3A5C] bg-[#DCE4F0] text-[#1A2A3A]' : 'border-[#D0D8E5] text-[#4A5A6A] hover:bg-[#E8EDF5]'}`}
                      >
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div>
                    <label className="text-[12px] font-medium text-[#4A5A6A] block mb-1.5">테마</label>
                      ...
                      {THEMES.map((t) => (
                        <button
                          ...
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-medium transition-all ${
                            theme === t.id
                              ? 'bg-[#DCE4F0] text-[#1A2A3A] border border-[#B0C0D5]'
                              : 'text-[#4A5A6A] hover:bg-[#E8EDF5] border border-transparent'
                          }`}
                        >
                  ...
                  <div>
                    <label className="text-[12px] font-medium text-[#4A5A6A] block mb-1.5">슬라이드 수</label>
                    <input
                      type="range"
                      min={3}
                      max={15}
                    ...
                    />
                    <span className="text-[12px] text-[#6A7A8A]">{slideCount}장</span>
                  </div>
                </div>
              </div>
              ...
              {slides.length > 0 && (
                <div>
                  <h3 className="text-[13px] font-bold text-[#1A2A3A] mb-3">슬라이드 목록</h3>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {['더 간결하게', '내용 보강', '성경 구절 추가', '비유/예화 추가'].map((label) => (
                      <button
                        ...
                        disabled={refining}
                        className="px-2.5 py-1 rounded-lg text-[11px] font-medium bg-[#E8EDF5] text-[#4A5A6A] hover:bg-[#DCE4F0] disabled:opacity-40 transition-all"
                      >
                        {label}
                      </button>
                    ))}
                  ...
                  <div className="flex gap-2 mb-4">
                    <input
                      ...
                      className="flex-1 px-3 py-2 rounded-lg border border-[#D0D8E5] text-[12px] text-[#1A2A3A] focus:outline-none focus:border-[#1B3A5C]"
                    />
                    <button
                      onClick={() => handleRefine(refineInput)}
                      disabled={refining || !refineInput.trim()}
                      className="px-3 py-2 rounded-lg text-[12px] font-medium bg-[#1B3A5C] text-white hover:bg-[#0F2B4F] disabled:opacity-40 transition-all"
                    >
                      {refining ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : '수정'}
                    </button>
                  </div>

                  <h3 className="text-[13px] font-bold text-[#1A2A3A] mb-3">슬라이드 편집</h3>
                  <div className="space-y-3">
                    <div>
                      <label className="text-[11px] font-medium text-[#4A5A6A] block mb-1">제목</label>
                      <input
                        type="text"
                        value={currentSlide.title}
                        onChange={(e) => updateSlide(activeIndex, { ...currentSlide, title: e.target.value })}
                        className="w-full px-3 py-2 rounded-lg border border-[#D0D8E5] text-[13px] text-[#1A2A3A] focus:outline-none focus:border-[#1B3A5C]"
                      />
                    </div>
                    <div>
                      <label className="text-[11px] font-medium text-[#4A5A6A] block mb-1">레이아웃</label>
                      <select
                    ...
                    <div>
                      <label className="text-[11px] font-medium text-[#4A5A6A] block mb-1">내용</label>
                      <div className="space-y-1.5">
                        {currentSlide.content.map((item, ci) => (
                          <div key={ci} className="flex gap-1">
                            <input
                              ...
                              className="flex-1 px-2.5 py-1.5 rounded-lg border border-[#D0D8E5] text-[12px] text-[#1A2A3A] focus:outline-none focus:border-[#1B3A5C]"
                            />
                            <button
                              onClick={() => {
                              ...
                              className="p-1.5 rounded text-[#8090A0] hover:text-red-500"
                            >
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 text-[12px] text-[#1B3A5C] font-medium hover:text-[#0F2B4F]"
                        >
                  ...
                  <div className="mt-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <h3 className="text-[13px] font-bold text-[#1A2A3A]">텍스트 스타일</h3>
                      <button
                        onClick={() => {
                          pushHistory()
                        ...
                        className="px-2.5 py-1 rounded-lg text-[11px] font-bold bg-[#1B3A5C] text-white hover:bg-[#0F2B4F] transition-all"
                      >
                        전체 적용
                      </button>
                    </div>
                  ...
                  {(currentSlide.coreMessage || currentSlide.speakerNotes || currentSlide.visualRecommendation || currentSlide.designNote) && (
                    <div className="mt-5 space-y-3">
                      <h3 className="text-[13px] font-bold text-[#1A2A3A] flex items-center gap-1.5">
                        <Sparkles className="w-3.5 h-3.5 text-[#1B3A5C]" />
                        AI 분석 정보
                      </h3>

                      {currentSlide.coreMessage && (
                        <div className="bg-[#F0F6FF] border border-[#D0D8E5] rounded-xl p-3">
                          <p className="text-[10px] font-bold text-[#1B3A5C] uppercase tracking-wide mb-1.5">💡 핵심 메시지</p>
                          <p className="text-[12px] text-[#1A2A3A] leading-relaxed">{currentSlide.coreMessage}</p>
                        </div>
                      )}

                      {currentSlide.speakerNotes && (
                        <div className="bg-[#E8F0FE] border border-[#B0C8E8] rounded-xl p-3">
                          <p className="text-[10px] font-bold text-[#1B3A5C] uppercase tracking-wide mb-1.5">🎤 발표자 스크립트</p>
                          <p className="text-[11px] text-[#1A2A3A] leading-relaxed whitespace-pre-wrap">{currentSlide.speakerNotes}</p>
                        </div>
                      )}
                      ...
                      {currentSlide.visualRecommendation && (
                        <div className="bg-[#f5f0ff] border border-[#ddd0ff] rounded-xl p-3">
                          ...
                          <p className="text-[11px] text-[#1A2A3A] leading-relaxed">{currentSlide.visualRecommendation}</p>
                        </div>
                      )}
