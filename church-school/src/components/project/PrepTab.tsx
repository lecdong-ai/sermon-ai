...
import { Loader2, Sparkles, Plus, X, Users, Settings2, Quote, FileText, BookOpen } from 'lucide-react'
import { ProjectDetail, CongregationProfile, DEFAULT_CONGREGATION_PROFILE, AGE_GROUP_OPTIONS, FAITH_MATURITY_OPTIONS } from '@/lib/project/types'
import { AppSectionHeader, PrepVersionHistory } from '@/components/project/shared'
import ProjectContextRow from '@/components/project/shared/ProjectContextRow'
import type { PrepVersion } from '@/lib/project/johnVersionData'
import { PREP_VERSIONS, RECENT_ACTIVITY } from '@/lib/project/johnVersionData'
...
interface PrepData {
  ...
  memosByPassage: Record<string, { label: string; text: string; tags: string[] }>
}
    ...
    try {
      ...
      if (studyData) {
        setPrepData(prev => ({
          ...
          memosByPassage: studyData.memosByPassage && Object.keys(studyData.memosByPassage).length > 0
            ? studyData.memosByPassage
            : prev.memosByPassage,
        }))
      }
    } catch (e) {
    ...
    if (raw) {
      ...
      setPrepData(prev => {
        ...
        return {
          ...
          memosByPassage: prev.memosByPassage && Object.keys(prev.memosByPassage).length > 0
            ? prev.memosByPassage
            : (savedPrep.memosByPassage || {}),
        }
      })
      if (_savedAt) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <PrepContextHeader
        ...
        lastSaved={lastSavedDisplay}
        onGoToStudy={() => router.push(`/projects/${project.id}?tab=study`)}
        onGoToManuscript={() => router.push(`/projects/${project.id}?tab=manuscript`)}
      />
