...
import { EventRecord, ApplicationInput, CustomField } from '@/types/event'
...
export default function EventApplyPage() {
  ...
  const checkSibling = async (phone: string) => {
    ...
    const res = await fetch(`/api/events/${token}/check?phone=${encodeURIComponent(phone)}`)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-navy-50 to-white px-4">
        <div className="text-center">
          ...
          <Link href="/" className="text-navy-500 underline">홈으로</Link>
        </div>
      </div>
    )
  }
